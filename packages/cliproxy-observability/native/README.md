...
`harness/results/` contains local Docker measurements, not CT101 capacity proof. `measure-hooks.sh` temporarily adds a test to the exact upstream source to measure host clones and native RPC; it never rebuilds the serving binary. Client first-byte time is separate from semantic TTFT. The initial callback p99 was 18.52 ms at concurrency 8 / 65,579-byte requests / 2 CPUs, exceeding the provisional 1 ms target. Added RAM alone cannot fix callback tail latency.

Allocate at least 4096 MiB RAM to CT101 before activation. Initial combined stock/plugin/exporter RSS stayed below 120 MiB at the tested load, leaving substantial headroom. Initial read-only preflight found about 131 GiB host RAM available and 26 GiB container root space available; recheck before applying resources. The 2 GiB outbox plus free-space reserve fit the existing 32 GiB root at that preflight.
